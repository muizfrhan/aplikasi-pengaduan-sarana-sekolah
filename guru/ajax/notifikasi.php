...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_read'], $_POST['id'])) {
    $id = (int)$_POST['id'];
    execute("UPDATE notifications SET is_read = 1 WHERE id = ? AND (user_id = 0 OR user_id = ?)", [$id, $userId]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_all_read'])) {
    ...
    execute("UPDATE notifications SET is_read = 1 WHERE (user_id = 0 OR user_id = ?) AND is_read = 0", [$userId]);
