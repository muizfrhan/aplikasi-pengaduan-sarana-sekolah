...
$unreadCount = (int)fetch(query("SELECT COUNT(*) as c FROM notifications WHERE user_id = ? AND is_read = 0", [$userId]))['c'];
$notifList = query("SELECT id, judul, pesan, link, jenis, created_at, is_read FROM notifications WHERE user_id = ? ORDER BY id DESC LIMIT 15", [$userId]);
$maxNotifId = 0;
$notifItems = [];
...
$allCount = (int)fetch(query("SELECT COUNT(*) as c FROM notifications WHERE user_id = ?", [$userId]))['c'];
...
<?php
$userPage = basename($_SERVER['SCRIPT_NAME'], '.php');
$userPages = [
    'index'          => ['icon' => 'fa-home',             'title' => 'Dashboard'],
    'buat_pengaduan' => ['icon' => 'fa-plus-circle',      'title' => 'Buat Pengaduan'],
    'riwayat'        => ['icon' => 'fa-history',          'title' => 'Riwayat Pengaduan'],
    'buat_testimoni' => ['icon' => 'fa-comment-dots',     'title' => 'Kirim Testimoni'],
    'profil'         => ['icon' => 'fa-user',             'title' => 'Profil Saya'],
    'pesan_password' => ['icon' => 'fa-envelope',         'title' => 'Pesan Password'],
    'reset_password' => ['icon' => 'fa-key',              'title' => 'Reset Password'],
];
$userPageInfo = $userPages[$userPage] ?? ['icon' => 'fa-home', 'title' => 'Dashboard'];
?>
                <div class="page-indicator d-none d-md-flex align-items-center gap-2">
                    <span class="page-indicator-icon"><i class="fas <?= $userPageInfo['icon'] ?>"></i></span>
                    <span class="page-indicator-text"><?= $userPageInfo['title'] ?></span>
                </div>
            </div>
                            ...
                            <div class="dropdown-profile-header">
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <div class="dropdown-profile-info">
                                        ...
                                        <div class="dropdown-profile-role">Siswa</div>
                                    </div>
                                </div>
                            </div>
                        </li>
                        <li><hr class="dropdown-divider"></li>
                        <li><a class="dropdown-item" href="profil.php"><span class="dropdown-icon bg-primary-subtle"><i class="fas fa-user"></i></span>Profil Saya</a></li>
                        <li><hr class="dropdown-divider"></li>
...
var NOTIF_POLL_URL = '../user/ajax/notifikasi.php';
...
function showNotifToast(item) {
    var m = { pengaduan_baru:'info', pengaduan_diproses:'info', pengaduan_selesai:'success', pengaduan_ditolak:'error', pesan_password:'info', pesan_baru:'info', reset_password:'warning', password_baru:'success' };
